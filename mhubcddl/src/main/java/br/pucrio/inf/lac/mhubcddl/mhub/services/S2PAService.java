package br.pucrio.inf.lac.mhubcddl.mhub.services;
...
import br.pucrio.inf.lac.mhubcddl.R;
import br.pucrio.inf.lac.mhubcddl.mhub.broadcastreceivers.BatteryReceiver;
import br.pucrio.inf.lac.mhubcddl.mhub.broadcastreceivers.BroadcastMessage;
...
import br.pucrio.inf.lac.mhubcddl.mhub.models.base.LocalMessage;
...
import br.pucrio.inf.lac.mhubcddl.mhub.models.queries.S2PAQuery;
import br.pucrio.inf.lac.mhubcddl.mhub.s2pa.base.Technology;
import br.pucrio.inf.lac.mhubcddl.mhub.s2pa.base.TechnologyListener;
import br.pucrio.inf.lac.mhubcddl.mhub.s2pa.technologies.ble.BLETechnology;
import br.pucrio.inf.lac.mhubcddl.mhub.s2pa.technologies.bt.BTTechnology;
import br.pucrio.inf.lac.mhubcddl.mhub.s2pa.technologies.internal.InternalTechnology;
...
public class S2PAService extends Service implements TechnologyListener {
    ...
    private void bootstrap() {
        ...
        Technology temp1 = new BLETechnology(ac);
        boolean exist1 = temp1.initialize();
        if (exist1) {
            technologies.append(BLETechnology.ID, temp1);
            temp1.setListener(this);
            temp1.enable();
        }

        Technology temp2 = BTTechnology.getInstance(ac);
        boolean exist2 = temp2.initialize();
        if (exist2) {
            technologies.append(BTTechnology.ID, temp2);
            temp2.setListener(this);
            temp2.enable();
        }

        Technology temp3 = InternalTechnology.getInstance(ac);
        boolean exist3 = temp3.initialize();
        if (exist3) {
            technologies.append(InternalTechnology.ID, temp3);
            temp3.setListener(this);
            temp3.enable();
        }
    ...
    public void onMObjectValueRead(MOUUID mobileObject, Double rssi, String serviceName, Double[] values) {
        ...
        sensorData.setLatitude(0d);  //TODO:lcmuniz
        sensorData.setLongitude(0d);  //TODO:lcmuniz
    ...
    public void onMObjectValueRead(MOUUID mobileObject, Double rssi, String serviceName, SensorDataExtended sensorData) {
        ...
        ///SensorDataExtended sensorData = new SensorDataExtended();
        sensorData.setMouuid(mobileObject.toString());
        sensorData.setSignal(rssi);
        sensorData.setSensorName(serviceName);
        sensorData.setAction(SensorData.READ);

        sensorData.setLatitude(sensorData.getSourceLocation().getLatitude());
        sensorData.setLongitude(sensorData.getSourceLocation().getLongitude());
